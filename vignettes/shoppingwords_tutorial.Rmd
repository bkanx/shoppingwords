...
The review_**`reviews.rda`** data is the core component of the shoppingwords package, designed for text analysis, sentiment scoring, or NLP tasks (e.g., customer feedback mining). The **`data()`** function provides a convenient way to load datasets by specifying their name.

### Loading Reviews Dataset

The package stores **`reviews.rda`** data in an optimized RDA format for fast access. The **`data()`** function loads the main data of product reviews from an RDA file stored within the package. 
...
corrected_words <- data("corrected_words", package = "shoppingwords") 
```
...
corrected_words <- get("corrected_words")  # Explicitly retrieve it as a data frame if needed
...
The `data()` function also retrieves a predefined list of 92 stopwords from a RDA file stored within the package. These stopwords are commonly found in Turkish shopping-related text and are useful for text preprocessing and analysis. **`stopwords_tr`** is a data specific to Turkish stopwords.
...
stopwords_tr |> dplyr::slice(38:41)
```
...
The `data()` function also retrieves predefined shopping-related phrases from the **`phrases.rda`** file stored within the package. 
...
#| message: false
#| echo: false
shopping_phrases <- data("phrases", package = "shoppingwords")
