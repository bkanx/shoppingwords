...
split <- initial_split(reviews_test, prop = 0.8, strata = emotion)
...
diagnostic_set <- validation_data |>
  filter(text %in% overlaps) # contains comments found in both reviews_test and reviews (the 170 common ones)
...
|------------------------------|--------|-------------|
| `reviews`                | RDA    | Turkish raw review data for sentiment analysis and NLP tasks. |
| `stopwords_tr`           | RDA   | A list of Turkish stopwords including shopping-based terms for text preprocessing. |
| `phrases`                | RDA    | A data containing predefined Turkish phrases for analysis. |
| `reviews_test`               | RDA   | Sample test data including sentence-based polarity for further NLP tasks (In Turkish). |
...
The `shoppingwords` package also includes a **reviews_test** data for text mining and NLP tasks. This data contains user-generated reviews labeled as **positive (`p`)** or **negative (`n`)**, being different than reviews data and making it useful for training models in sentiment classification (See Table 3).
...
_Table 3: Overview of the **`reviews_test`** dataset._

### **Loading the reviews_test**
...
data("reviews_test", package = "shoppingwords")
head(reviews_test, 3)
```
```{r}
# English translation
...
The reviews_test can be used in various ways such as capturing the negative expressions or predciting sentence-level polarity using the `reviews`. 
